...
Ext.define('mobile.undertake.CompletedMatters', {
	extend : 'HT.PageList',
	...
	constructor : function(conf) {
		...
		this.store = Ext.create('HT.Store', {
			model : 'modelItem',
			url : __ctx + '/platform/mobile/mobileTask/completedMatters.ht'
		});
		...
		Ext.apply(conf, {
			...
			itemTpl : Ext
					.create(
							...
							'<div class="top">',
							'<div class="name">{subject}<span>{processName}{status}{creator}<span class="time">{createtime}<div>{endTime}</div></span></span></div>',
							'</div>', '</div>',
