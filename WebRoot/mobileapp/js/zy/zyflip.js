	...
	var zyFlip = function(selector,dir,endFun,lock,sfun){
		var self = this;
		self._locked= lock;
		self.endFun = endFun || null;
		self.startFun = sfun || null;
		self.nodes = [];
		//V:vertical
        //H:horizontal
		self.dir = dir||"H";
	...
	zyFlip.prototype = {
		handleEvent: function(event) {
			var self = this;
		//	uexLog.sendLog("event=="+event.type);
			switch (event.type) {
				case $E.start:
        ...
        _touchStart: function(event) {
        	//uexLog.sendLog("start=======");
            var self = this;
			//self.stime = new Date().getTime();
            if(self._locked)
            {
        ...
        _touchEnd: function(event) {
			...
			//uexLog.sendLog("self.stime=="+self.stime);
            if(self._locked)
            {
           		event.preventDefault();
	            event.stopPropagation();
            ...
            if(self.endFun)
            setTimeout(self.endFun(newPoint), 500);
        },
        ...
        destroy: function() {
            ...
            self.element.parentNode.removeEventListener(touchStartEvent, self);
            self.element.parentNode.removeEventListener(touchMoveEvent, self);
