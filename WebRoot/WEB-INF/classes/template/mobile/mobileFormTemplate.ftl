						...
						<#list subTables as subTable>
							 ...
							 <div class="ub ub-f1 uba3px b-e1e1e1 uof-x scrollers" formtype="subTable">
	                            <table class="ub scrollerW">
	                                <tr class="ub ub-f1">
	                                	<td class="ubr uba ub b-ddd t-bla ub-ac uinn-a9 c-gra2 w3">
	                                      	  序号 
	                                    </td>
	                                	<td class="ubr ub-trb ub b-ddd t-bla ub-ac uinn-a9 c-gra2 mwd10"  ng-repeat="filedName in [${getSubFiledDesc(subTable.fieldList)}] track by $index">
	                                		{{filedName}}
	                                	</td>
									</tr>
									 <tr class="ub ub-f1" ng-repeat="trs in sub['${subTable.tableName?lower_case}']" ng-click="editSubTableCol('${subTable.tableName?lower_case}',$index)">
									    <td class="ubr ub-rbl ub b-ddd t-bla ub-ac uinn-a9 c-wh w3" >
	                                        		{{$index+1}}
	                                    </td>
									 	<td class="ubr ub-rb ub b-ddd t-bla ub-ac uinn-a9 c-wh mwd10 ut-s"  ng-repeat="filedName in  [${getSubFiledNames(subTable.fieldList)}]">
	                                        		<span ng-bind-html="trs[filedName] | turn_to_subdata | turn_to_selectText:'item.{{filedName}}'" class="mar-r0625""></span>
	                                    </td>
	                                 </tr>
								</table>
							 </div>
...
<#list subTables as subTable>
 <div id="${subTable.tableName?lower_case}" class="wrapper uhide z-index9999" >
 	...
 	<div class="uh t-bla ub ub-ac b-e1 ubb h-275 c-wh subAddTitle  sl_header fixed">
