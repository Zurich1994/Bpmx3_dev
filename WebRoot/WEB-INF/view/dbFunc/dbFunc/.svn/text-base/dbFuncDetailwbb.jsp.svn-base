...
<head>
	...
	<script type="text/javascript">
		...
		function init(){
			...
			var fields=${fields};
			var  conditionField=fields.conditionField;
			...
			if(fields.setField)initSetField(fields.sortField);
		}
...
//				objContainer.append(getConditionField(c.field,c.comment,c.dbType));
				objContainer.append(getConditionField(c.comment,c.comment,c.dbType));
				var curTr = $("#condition"+c.comment),
					curValTr = $("#conditionVal"+c.omment);
...
//				objContainer.append(getRtnField(c.field,c.comment));
				objContainer.append(getRtnField(c.comment,c.comment));
			}
		};
		...
		function initSortField(sortField){
			...
			for(var i=0,c;c=fieldObj[i++];){
				...
				//tr.attr('id','sort'+c.comment);
				//tr.attr('name',c.comment);
				tr.css('display','');
				if(c.comment=='ASC'){
...
//				$('td#fieldName',tr).text(c.field);
				$('td#fieldName',tr).text(c.comment);
				objContainer.append(tr);
			}
		};
		function initSetField(setField){
		$.ligerDialog.warn("init res",'result');
			//var fieldObj=eval("("+resultField+")"),
			var	objContainer=$("#trSetField");
			objContainer.empty();
//			if(!fieldObj)return;
		//	if(!resultField)return;
			for(var i=0,c;c=setField[i++];){
				objContainer.append(getRtnField(c.fieldName,c.comment));
			}
		};
		var curField;
		/*
		...
		<div id="defLayout" >
	            ...
	            <div id="fieldSetting" position="right"  title="字段设置">
					...
					<a href='#' class='button'  onclick="selectForm()" ><span class="icon ok"></span><span >确定</span></a>
					 -->	  
				  		<a href='#' class='button' style='margin-left:10px;' onclick="dialog.close();"><span class="icon cancel"></span><span >关闭</span></a>
					</div>
