...
<head>
	...
	<script type="text/javascript">
		...
		function buildTreeJson(){
			...
			displayStr="[{\"field\":\""+treeId+"\",\"comment\":\""+parentId+"\"}]" ;
		...
		<div id="defLayout" >
			    ...
			    <div class="content" >
			       <table cellpadding="1" class="table-grid table-list" cellspacing="1">
						...
						<c:forEach var="col" items="${tableparcel }" varStatus="status">
						<c:set var="clsName"  ><c:choose><c:when test="${status.index%2==0}">odd</c:when><c:otherwise>even</c:otherwise></c:choose> </c:set>
						<tr class="${clsName}">
							<td>
								<c:choose>
	            					<c:when test="${style==0 }">
										<input type="checkbox" name="fieldName"  class="pk"  value="${col.parcel_name }"  d>
									</c:when>
									<c:otherwise>
										<input type="checkbox" name="fieldName" class="pk"  value="${col.parcel_name }" id="${col.parcel_name }" >
									</c:otherwise>
								</c:choose>
							</td>
							<td nowrap="nowrap">
							...
							<td>
								<input type="text" name="comment" class="inputText" value="${col.parcel_alias }">
							</td>
						</tr>
						</c:forEach>
					</table>  	
	            ...
	            <div id="fieldSetting" position="right"  title="字段设置">
	            	...
	            	<c:choose>
	            	<c:when test="${(id!=0)&&(func_type_long ==4) }">
...
<!-- 											<input type="checkbox" name="treeReturn" checked="checked" />返回 -->
			          ...
			          <c:choose>
				          	<c:when test="${(func_type_long ==2)||(func_type_long ==3||(func_type_long ==4)) }">
					...
					<div class="tab-layout trConditionField">
	          			<table cellpadding="1" class="table-grid table-list" cellspacing="1">
	          				...
	          				<tbody id="trConditionField">
	          					<c:forEach items="${ dbfunc.conditionList}" var="field">
