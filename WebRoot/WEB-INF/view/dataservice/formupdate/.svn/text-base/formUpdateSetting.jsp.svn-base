...
<head>
	...
	<script type="text/javascript">
		...
		alert("3");
			...
			//alert(fields.displayField);
			//alert(fields.conditionField);
			//alert(fields.updateField);
			//alert("4");
			...
			//alert("6");
			if(fields.updateField)initResultField(fields.updateField);
			//alert("7");
		...
		//初始化返回字段
		function initResultField(updateField){
			alert("initResultField");
			var fieldObj=eval("("+updateField+")"),
				objContainer=$("#trRtnField");
			objContainer.empty();
			if(!fieldObj)return;
			for(var i=0,c;c=fieldObj[i++];){
				objContainer.append(getRtnField(c.field,c.comment,c.dbType));
				var curTr = $("#rtn"+c.field),
				curValTr = $("#rtnVal"+c.field);
				$("select.rtnCondition",curTr).val(c.condition);
				
				$("select[name='rtnChangeDefalut']",curValTr).val(c.defaultType);
				rtnChangeDefault($("select[name='rtnChangeDefalut']",curValTr));
				$("textarea[name='rtnDefaultValue']",curValTr).val(c.defaultValue);
			}
		};
		...
		function getConditionField(fieldName,comment,dbType){
			...
			sb.append("<option value='2'>固定值</option><option value='3'>脚本</option><option value='6'>流程变量</option></select></td>");
			...
			sb.append("></textarea><select name='flowValue' class='flowValue' style='display:none;' >");
			sb.append("<option value='flowValue1'>flowValue1</option>");
			sb.append("<option value='flowValue2'>flowValue2</option><option value='flowValue3'>flowValue3</option></select></td>");
			sb.append("</tr>");
			return sb.toString();
		}
		...
		function changeDefault(obj){
			...
			var selObj=$("select[name='flowValue']",objTr);
			switch(val){
				case "1":
					txtObj.hide();
					linkObj.hide();
		...
		function rtnChangeDefault(obj){
			alert("rtnChangeDefault");
			var val=$(obj).val();
			var objTr=$(obj).parents("tr");
			var txtObj=$("textarea[name='rtnDefaultValue']",objTr);
			var linkObj=$("a[name='rtnBtnScript']",objTr);
			var selObj=$("select[name='rtnFlowValue']",objTr);
			switch(val){
				case "1":
					txtObj.hide();
					linkObj.hide();
		...
		function getRtnField(fieldName,comment,dbType){
			alert("getRtnField");
			var db=getRtnSelect(dbType,fieldName,comment);
			var sb=new StringBuffer();
			sb.append("<tr id='rtn"+ fieldName +"' name='"+fieldName+"' comment='"+comment+"' class='trRtnCondition'  >");
			sb.append("<td >"+fieldName+"</td>");
			sb.append("<td >"+db+"</td>");
			sb.append("<td >"+comment+"</td>");
			...
			sb.append("<a href='#' class='link del' onclick='delConditionTr(this)' >删除</a></td>");
			sb.append("</tr>");
			sb.append("<tr id='rtnVal"+ fieldName +"'>");
			sb.append("<td>默认值</td>");
			sb.append("<td><select name='rtnChangeDefalut' onchange='rtnChangeDefault(this)'>");
			sb.append("<option value='1'>参数传入</option>");
			sb.append("<option value='2'>固定值</option><option value='3'>脚本</option><option value='6'>流程变量</option></select></td>");
			sb.append("<td colspan='2'><a style='display:none;' href='#' name='rtnBtnScript' class='link var' title='常用脚本' onclick='selectScript(this)'>常用脚本</a>");
			sb.append("<textarea name='rtnDefaultValue' cols='40' rows='3' ");
			sb.append("		style='display:none;' ");
			sb.append("></textarea><select name='rtnFlowValue' style='display:none;' >");
			sb.append("<option value='1'>flowValue1</option>");
			sb.append("<option value='2'>flowValue2</option><option value='3'>flowValue3</option></select></td>");
			sb.append("</tr>");
			return sb.toString();
		}
		
		function getConditionSelect(dbType,name,comment){
			alert("getConditionSelect");
			var sb=new StringBuffer();
		...
		function getRtnSelect(dbType,name,comment){
			alert("getRtnSelect");
			var sb=new StringBuffer();
			sb.append("<select class='rtnCondition' name='"+name+"' dbType='"+dbType+"' comment='"+comment+"'>");
			switch(dbType){
				case "varchar":
					sb.append("<option value='='>等于</option>");
					sb.append("<option value='like'>LIKE</option>");
			...
			if(divParent.attr('class').indexOf('condition')>=0){
				...
				java脚本区域
					...
					if($('.btnTree')!=undefined && $('.btnTree').length>0){
						...
						alert("setConditionField(false)");
					}else
						setConditionField(true) ;
						alert("setConditionField(true)");
				}
				return ;
			}
		...
		function setReturnField(){
			...
			$("input:checkbox[name='fieldName']:checked").each(function(){
				...
				if(obj.length==0){
					var tr=getRtnField(fieldName,comment,dbType);
					objContainer.append(tr);
				}
			});
		}
		...
		function getCondition(){
			...
			$("#trConditionField").children("tr.trCondition").each(function(){
				...
				var selDefault=$("select[name='changeDefalut']",trDefault).val();   //数据来源类型
				//alert("selDefault:"+selDefault);
				var txtDefault=$("textarea[name='defaultValue']",trDefault).val();  //textarea的值
				//alert("txtDefault:"+txtDefault);
				var flowValue = $("select[name='flowValue']",trDefault).attr("value"); 
				//alert("flowValue:"+flowValue);            
				...
				if(selDefault=="1"){
					obj.defaultValue="";
				}else if(selDefault=="6"){
					alter("6");
					obj.defaultValue="";
					obj.flowValue = flowValue;
				}else{
		...
		function buildListJson(){
			...
			$("#trRtnField").children("tr.trRtnCondition").each(function(){
				//tr.trRtnCondition
				//alert($("#trRtnField").children().length);
				//alert("trRtnCondition");
				var $tr = $("#trRtnField").children();
				var trLen = $("#trRtnField").children().length
				//for(var i=0,i<trLen;i++){
				//	alert($tr[i].innerHTML);
				//}
				//var fieldName=$(this).attr("name");
				//var comment=$(this).attr("comment");
				//var fieldName = $(".rtnCondition").attr("name");
				//var comment=$(".rtnCondition").attr("comment");
				//var dbType = $(".rtnCondition").attr("dbType");
				//var condition = $(".rtnCondition").attr("value");
				var trObj=$(this);
				var trDefault=trObj.next();
				var selObj=$('select.rtnCondition',trObj);
				var fieldName=selObj.attr("name");
				var comment=selObj.attr("comment");
				var condition=selObj.attr("value");
				var dbType=selObj.attr("dbType");
				...
				obj.dbType = dbType;
				obj.condition = condition;
				var selDefault=$("select[name='rtnChangeDefalut']",trDefault).val();
				var txtDefault=$("textarea[name='rtnDefaultValue']",trDefault).val();
				if(selDefault!="1" && txtDefault.trim()==""){
					rtn=false;
				}
				obj.defaultType=selDefault;
	    ...
	    function selectForm(){
			...
			var call=dialog.get("sucCall");  //函数传递 call相当subCall
			call(rerurnlist);
			dialog.close();
	    }
...
<body>
		<input type="text" name="defId" value="<%=request.getAttribute("defId") %>" />
		<input type="text" name="nodeId" value="<%=request.getAttribute("nodeId") %>"/>
		<div id="defLayout" >
	            ...
	            <div id="fieldSetting" position="right"  title="字段设置">
			          						...
			          						<tr id='conditionVal${field.fieldName}'>
			          							...
			          							<td>
			          								<select name='changeDefalut' onchange='changeDefault(this)'>
			          									...
			          									<option value="6" <c:if test="${field.defaultType=='6'}">selected</c:if>>流程变量</option>
			          								</select>
			          							</td>
			          							<td colspan="2">
			          							...
			          							<select name='flowValue' class='flowValue'>
			          								<option value="flowValue1">flowValue1</option>
			          								<option value="flowValue2">flowValue2</option>
			          								<option value="flowValue3">flowValue3</option>
			          							</select>
			          							</td>
			          						</tr>
			          					</c:forEach>
			          			...
			          			<table cellpadding="1" class="table-grid table-list" cellspacing="1">
			          				...
			          				<tbody id="trRtnField">
			          					<c:forEach items="${ bpmFormDialog.returnList}" var="field">
			          						<tr id='rtn${field.fieldName}' class='trRtnCondition' name='${field.fieldName}' comment='${field.comment}'>
			          							<td>${field.fieldName}</td>
			          							
			          							<td>
			          								<select class='rtnCondition' name='${field.fieldName}' dbType='${field.fieldType}' comment='${field.comment}' >
			          								<c:choose>
			          									<c:when test="${field.fieldType=='varchar'}">
				          									<option value='=' <c:if test="${field.condition=='='}">selected</c:if> >等于</option>
															<option value='like' <c:if test="${field.condition=='like'}">selected</c:if>>LIKE</option>
			          						...
			          						<tr id='rtnVal${field.fieldName}'>
			          							<td>默认值</td>
			          							<td>
			          								<select name='rtnChangeDefalut' onchange='rtnChangeDefault(this)'>
			          									<option value="2" <c:if test="${field.defaultType=='2'}">selected</c:if>>固定值</option>
			          									<option value="3" <c:if test="${field.defaultType=='3'}">selected</c:if>>脚本</option>
			          									<option value="6" <c:if test="${field.defaultType=='6'}">selected</c:if>>流程变量</option>
			          								</select>
			          							...
			          							<td colspan="2">
			          							<a <c:if test="${ field.defaultType=='2'}">style='display:none;'</c:if>  href='#' name='rtnBtnScript' 
			          							class='link var' title='常用脚本' onclick='selectScript(this)'>常用脚本</a><br>
			          							<textarea name='rtnDefaultValue' cols='40' rows='3' >${field.defaultValue}</textarea>
			          							<select  name='rtnFlowValue'>
			          								<option value="flowValue1">flowValue1</option>
			          								<option value="flowValue2">flowValue2</option>
			          								<option value="flowValue3">flowValue3</option>
			          							</select>
					...
					<input type="hidden" name="updatefield" id="updatefield" value="${bpmFormDialog.resultfield}"/>
