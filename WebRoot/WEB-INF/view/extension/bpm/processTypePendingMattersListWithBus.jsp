...
<script type="text/javascript" src="${ctx}/js/hrbeu/platform/bpm/BpmTaskExeAssignDialog.js"></script>
	...
	<div class="hide-panel">
		<div class="panel-top">
			...
			<div class="panel-search" >
				<form id="searchForm" method="post" action="pendingMattersListWithBus.ht?porIndex=${porIndex}&tabIndex=${tabIndex}">
					<ul class="row">
						<li>
					...
					<div class="row_date">
						...
						<select name="Q_status_N"   class="selectSearch">
							...
							<option value="4" <c:if test="${param['Q_status_N'] == '5'}">selected</c:if>>已撤销</option>
							<option value="5" <c:if test="${param['Q_status_N'] == '6'}">selected</c:if>>已驳回</option>
						</select>
						</li>
					</ul>
		...
		<div class="panel-body">
			    ...
			    <display:table name="taskList" id="taskItem" requestURI="pendingMattersListWithBus.ht" sort="external" cellpadding="1" cellspacing="1"  class="table-grid">
					...
					<display:column title="事项名称"  sortable="true" sortName="subject"  style="text-align:left;" >
							<c:choose>
									<c:when test="${taskItem.description eq '36'}">
										<a name="subject"  href="#" onclick="javascript:restartTask(${taskItem.id})" title='${taskItem.subject}'  <c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if> >${f:subString(taskItem.subject)}</a>
									</c:when>
								<c:otherwise>
										<a name="subject"  href="#" onclick="javascript:executeTask(${taskItem.id})" title='${taskItem.subject}' <c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if><c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if> >${f:subString(taskItem.subject)}</a>
								</c:otherwise>
							</c:choose>
