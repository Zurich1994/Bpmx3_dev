...
<div class="panel">
 ...
 <tbody>
  ...
  <tr>
   <td style="width:20%;word-break:break-all" class="formTitle" nowrap="nowrap" align="right">服务类型:</td>
    ...
    <select name="serviceType" validate="{empty:false}" lablename="serviceType" controltype="select">  
                	<%List<String> list = (List)request.getAttribute("ServiceType");
  ...
  <tr>
   ...
   <td style="width:80%" class="formInput"><span style="display:inline-block;padding:2px;" class="selectinput" name="editable-input"><select name="serviceState" validate="{empty:false}" lablename="serviceState" controltype="select"><option selected="selected" value="启用" <c:if test='${flowwebservice.serviceState=="启用"}'>selected='selected'</c:if>>启用</option><option value="未启用" <c:if test='${flowwebservice.serviceState=="未启用"}'>selected='selected'</c:if>>未启用</option><option value="禁止" <c:if test='${flowwebservice.serviceState=="禁止"}'>selected='selected'</c:if>>禁止</option><option value="暂停" <c:if test='${flowwebservice.serviceState=="暂停"}'>selected='selected'</c:if>>暂停</option></select></span></td>
  </tr>
  <tr>
   <td style="width:20%;word-break:break-all" class="formTitle" nowrap="nowrap" align="right">流程ID:</td>
   <td style="width:80%" class="formInput"><input name="defid" validate="{number:true,maxIntLen:18,maxDecimalLen:0}" showtype="{'coinValue':'','isShowComdify':0,'decimalValue':0}" value="${flowwebservice.defid}" readonly="readonly" /></td>
  </tr>
 </tbody>
</table>
			</div>
