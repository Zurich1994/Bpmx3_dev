...
<script type="text/javascript">
	...
	function selectTable(){
		var obj=$("#newjsprelationItem tr.over");
		if(obj.length>0){
	...
	<div id="defLayout">
		<div id="centerLayout" position="center"  style="overflow: auto;">
			<div class="panel-top" usesclflw="true">
				...
				<form id="searchForm" method="post" action="dialog.ht">
					<div class="row">
						<span class="label">表单ID:</span><input type="text" name="Q_FORMDEFID_L"  class="inputText" />
					</div>
				</form>
			</div>
			</div>
		<!--  
			...
			-->
			 <display:table name="newjsprelationList" id="newjsprelationItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				<display:column  title="表单id" style="text-align:left;width:200px;">
					<input  type="hidden" value="${newjsprelationItem.FORMDEFID },${newjsprelationItem.name }">
					${newjsprelationItem.id}
				</display:column>
				
				<display:column property="name" title="表单称" sortable="true" sortName="F_NAME">
				${newjsprelationItem.name }
				</display:column>
