			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm(obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						...
						window.location.href = "${ctx}/devicein/deviceinpack/deviceInfomation/list.ht";
					}
				});
			} else {
...
<div class="panel">
 ...
 <tbody>
  ...
  <tr>
   ...
   <td class="formInput" style="width:40%">
   		<span style="display:inline-block" name="editable-input" isflag="tableflag">
   			<input class="inputText" name="dev_Name" validate="{maxlength:50,required:true}" lablename="设备名称" isflag="tableflag" value="${deviceInfomation.dev_Name}" />
   		</span>
   </td>
   </tr>
  <tr>
