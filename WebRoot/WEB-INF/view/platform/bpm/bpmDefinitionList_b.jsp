	...
	<div class="panel-top">	
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link flowDesign" onclick="window.open('design_b.ht')"><span></span>拓扑图设计</a></div>
					<div class="l-bar-separator"></div>
			...
			<div class="panel-search">
				...
				<form id="searchForm" method="post" action="list_b.ht">	    
					<ul class="row">
		...
		<div class="panel-body">
			    ...
			    <display:table name="bpmnetworkmapList" id="bpmnetworkmapItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
					<display:column title="${checkAll}" media="html" style="width:30px;">
						  	<input type="checkbox" class="pk" name="defId" value="${bpmnetworkmapItem.defId}" 
						  			defKey="${bpmnetworkmapItem.defKey}" subject="${bpmnetworkmapItem.subject}" right_m_del="${bpmnetworkmapItem.authorizeRight.managementDel}">
					</display:column>
					...
					<display:column title="分类" sortable="true" sortName="typeId">
						<c:out value="${bpmnetworkmapItem.typeName}"></c:out>
					</display:column>
					...
					<display:column title="管理" media="html" style="width:50px;text-align:center" class="rowOps">
						...
						<c:if test="${bpmnetworkmapItem.authorizeRight.managementDel=='Y'}">
							<f:a alias="delProcess_b" href="del_b.ht?defId=${bpmnetworkmapItem.defId}" css="link del" >删除</f:a>
						</c:if>
						<c:if test="${bpmnetworkmapItem.authorizeRight.managementEdit=='Y'}">
							<f:a alias="flex1" href="design_b.ht?defId=${bpmnetworkmapItem.defId}" target="_blank" css="link flowDesign" >设计</f:a>
						</c:if>
						<c:if test="${bpmnetworkmapItem.authorizeRight.managementSet_new=='Y'}">
							<f:a alias="flex2" href="design2.ht?defId=${bpmnetworkmapItem.defId}" target="_blank" css="link flowDesign" >设置</f:a>
						</c:if>	
						<c:if test="${bpmnetworkmapItem.authorizeRight.managementShow=='Y'}">
							<f:a alias="flex3" href="design1.ht?defId=${bpmnetworkmapItem.defId}" target="_blank" css="link flowDesign" >查看</f:a>
						</c:if>
						<c:if test="${bpmnetworkmapItem.authorizeRight.managementShowZhanshi=='Y'}">
							<f:a alias="setBpm_b" href="design3.ht?defId=${bpmnetworkmapItem.defId}" target="_blank" css="link flowDesign" >展示</f:a>
						</c:if>
						
						<!--
					    暂时去掉分管授权
							<f:a alias="grantProcess" href="javascript:FlowRightDialog(${bpmnetworkmapItem.defId},0,'${bpmnetworkmapItem.defKey}')" css="link grant" ><spr:message code="menu.button.authorize"/></f:a>
						--><!--
						
						<c:if test="${bpmnetworkmapItem.status==0}">
							<f:a alias="publishProcess" href="deploy.ht?defId=${bpmnetworkmapItem.defId}" css="link deploy" >发布</f:a>
						</c:if>
						
                 --><%--  <c:if test="${bpmnetworkmapItem.authorizeRight.managementInternational=='Y'}">
							<c:if test="${bpmnetworkmapItem.status!=0}">
								<f:a alias="internationalization" href="${ctx}/platform/bpm/bpmFormLanguage/defList.ht?actdefid=${bpmnetworkmapItem.actDefId}" css="link comment" >国际化</f:a>
							</c:if>
						</c:if> --%>
						
						<!--<c:if test="${bpmnetworkmapItem.authorizeRight.managementClean=='Y'}">
							<a  href="javascript:cleanData('${bpmnetworkmapItem.defId}')" class="link clean" >清除数据</a>
					    </c:if>
					-->
					</display:column>
