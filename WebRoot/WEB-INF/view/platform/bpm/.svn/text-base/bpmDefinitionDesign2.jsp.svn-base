...
<%
	...
	String postUrl3 = basePath + "/platform/bpm/bpmDefinition/flexDefSave3.ht?isAjaxRequest=true";
	String postUrl0 = basePath + "/platform/bpm/bpmDefinition/flexDefSave02.ht?isAjaxRequest=true";
	String postUrl5 = basePath + "/platform/bpm/bpmDefinition/flexDefSave5.ht?isAjaxRequest=true";
	String postUrl16 = basePath + "/platform/bpm/bpmDefinition/flexDefSave4.ht?isAjaxRequest=true";
	...
	if(defIdObj != null && !defIdObj.toString().equals("0")){
		...
		loadDateUrl = basePath + "/platform/bpm/bpmDefinition/flexGet_b.ht?temp=f&defId=" + defId;
	}
%>
  ...
  <style type="text/css" media="screen"> 
	...
	//供flex方向调用,获取提交数据的地址，设置业务IP
	function getCtxPath3(){		
		return document.getElementById("postUrl3").value;
	}
	
	//供flex方向调用,获取提交数据的地址，检验节点是否已绑定设备信息
	function getCtxPath16(){		
		return document.getElementById("postUrl16").value;
	}
	...
	//供flex方向调用,获取提交数据的地址,设置线的信息
	function getCtxPath5(){
		return document.getElementById("postUrl5").value;
	}
  ...
  <div>
    ...
    <textarea id="flagflex" style="display:none"><%=flagflex%></textarea>
  	...
  	<input id="postUrl5" type="hidden" value="<%=postUrl5%>" />   	
  	<input id="postUrl16" type="hidden" value="<%=postUrl16%>" /> 
