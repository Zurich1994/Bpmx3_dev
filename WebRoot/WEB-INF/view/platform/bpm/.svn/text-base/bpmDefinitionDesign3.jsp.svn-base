...
<%
	...
	if(defIdObj != null && !defIdObj.toString().equals("0")){
		...
		loadDateUrl = basePath + "/platform/bpm/bpmDefinition/flexGet_b.ht?temp=h&defId=" + defId;
	}
	
	String postUrl2 = basePath + "/platform/bpm/bpmDefinition/flexDefWritePage.ht?isAjaxRequest=true&defId=" + defId;	
	String equipmentConditionUrl=basePath + "/platform/bpm/bpmDefinition/equipmentCondition.ht?isAjaxRequest=true&defId=" + defId;	
%>
  ...
  <style type="text/css" media="screen"> 
	...
	function loadData(){
		...
		alert(document.getElementById('xmlRecord'));
		return str;
	}
	...
	//供flex方向调用,获取提交数据的地址,查看原子操作读页面——史欣慧
...
alert("getCtxPath10");
	...
	{
		return document.getElementById("equipmentConditionUrl").value;
	}
  ...
  <textarea id="flagflex" style="display:none"><%=flagflex%></textarea>
  	...
  	<input id="equipmentConditionUrl" type="hidden" value="<%=equipmentConditionUrl%>" />
  </div>
  <div id="flashContent">
