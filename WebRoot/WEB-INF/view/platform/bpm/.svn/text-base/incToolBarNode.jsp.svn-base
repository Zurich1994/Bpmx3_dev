		...
		<div class="toolBar">
			<c:choose>
							...
							<c:forEach items="${mapButton.button }" var="btn"  varStatus="status">
								<c:choose>
									...
									<c:when test="${btn.operatortype==18 }">
								     <!--  自定义-->
							         <div class="group" id="customizedBtnSet"><a class="link sendMessage" onclick="nextDialog('${btn.nodeid}','${btn.actdefid }')" ><span></span>${btn.btnname }</a></div>
									<div class="l-bar-separator"></div>
									</c:when>	
								</c:choose>
							</c:forEach>
				...
				<c:otherwise>
						...
						<div class="l-bar-separator"></div>
						<!--  自定义-->
						<div class="group" id="customizedBtnSet"></div>
						<div class="l-bar-separator"></div>
