	...
	<div class="panel-top">	
			<div class="panel-toolbar">
				<div class="toolBar">
					...
					<div class="l-bar-separator"></div>
					<div class="group"><a class="link export" onclick="exportChart()"><span></span>导出一图四表</a></div>
					<%} %>
					<!-- 
						 暂时去掉分管授权
						<div class="l-bar-separator"></div>
		...
		<div class="panel-body">
			    ...
			    <display:table name="bpmDefinitionList" id="bpmDefinitionItem" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" export="false"  class="table-grid">
					...
					<display:column property="subject" title="标题" sortName="subject" ></display:column>
					
					<display:column title="分类" sortName="typeId">
						<c:out value="${bpmDefinitionItem.typeName}"></c:out>
					</display:column>
					<display:column property="versionNo" title="版本号" sortName="versionNo" style="width:25px"></display:column>
					<display:column title="发布状态" sortName="status" style="width:45px;">
						<c:choose>
					...
					<display:column title="创建时间" sortName="createtime"  style="width:70px;">
					...
					<display:column title="管理" media="html" style="width:50px;text-align:center" class="rowOps">
						...
						<c:if test="${bpmDefinitionItem.authorizeRight.managementEdit=='Y'}">
							<f:a alias="flex" href="design.ht?defId=${bpmDefinitionItem.defId}&typeName=flowChart&status=${bpmDefinitionItem.status}" target="_blank" css="link flowDesign" >设计</f:a>
						...
						<c:if test="${bpmDefinitionItem.authorizeRight.managementSet=='Y'&& bpmDefinitionItem.status!=0&&bpmDefinitionItem.typeName!='事务图'}">
							<f:a alias="markovchain" href="${ctx}/Markovchain/Markovchain/markovchain/listbydefid.ht?defId=${bpmDefinitionItem.defId}" target="_self" css="link search" >设置马尔科夫链</f:a>
						</c:if>
				...
				<form id="frmDel">
					<table class="table-detail" cellpadding="0" cellspacing="0" border="0">
						<tr>
							...
							<td>
								<input class="catComBo" catKey="FLOW_TYPE" typeName="flowChart" valueField="typeId" catValue="" name="typeName" height="150" width="150"/>
							</td>
						</tr>
					</table>
