...
<html>
	<head>
		...
		<script type="text/javascript"
			src="${ctx}/js/hotent/platform/bpm/BpmTaskExeAssignDialog.js"></script>
		<script type="text/javascript"
			src="${ctx}/js/hotent/platform/bpm/SelectUtil.js"></script>
		<script type="text/javascript"
			src="${ctx}/js/hotent/platform/system/UserInfo.js"></script>
		<script type="text/javascript">
$(function(){
	...
	<body>
		<div class="panel">
				...
				<div class="panel-top">
					...
					<div class="panel-toolbar">
						<div class="toolBar">
							...
							<div class="l-bar-separator"></div>
							<div class="group">
								<f:a alias="batchApproval" css="link save" id="btnApprove"
									href="javascript:;" showNoRight="false"
									onclick="summaryBatch()">
									<span></span>批量审批</f:a>
							</div>
							<!-- 
			
					<div class="group"><f:a alias="batchApproval" css="link save" id="btnApprove" href="javascript:;" showNoRight="false" onclick="batOperator('approve');"><span></span>批量审批</f:a></div>
				 -->
						</div>
					</div>
					<div class="panel-search">
						<form id="searchForm" method="post"
							action="pendingMattersList.ht?porIndex=${porIndex}&tabIndex=${tabIndex}">
							<ul class="row">
								<input type="hidden" name="nodePath"
									value="${param['nodePath']}" title="流程分类节点路径"></input>
								<li>
									<span class="label">事项名称:</span>
									<input type="text" name="Q_subject_SUPL" class="inputText"
										value="${param['Q_subject_SUPL']}" />
								</li>
								<li>
									<span class="label">流程名称:</span>
									<input type="text" name="Q_processName_SUPL" class="inputText"
										value="${param['Q_processName_SUPL']}" />
								</li>
								<li>
									<span class="label">创 建 人:</span>
									<input type="hidden" id="creatorId" class="inputText"
										name="Q_creatorId_L" value="${param['Q_creatorId_L']}" />
									<input type="text" id="creator" name="Q_creator_SL"
										class="inputText" style="width: 110px;"
										value="${param['Q_creator_SL']}"
										onclick="selectUser('creatorId','creator');"
										readonly="readonly" />
									<input type="button" value="..." class="buttonSearch"
										onclick="selectUser('creatorId','creator');">
								</li>
							</ul>
							<ul class="row">
								<li>
									...
									<select name="Q_hasRead_S" class="selectSearch">
										...
										<option value="0"
											<c:if test="${param['Q_hasRead_S'] == '0'}">selected</c:if>>
											未读
										</option>
										<option value="1"
											<c:if test="${param['Q_hasRead_S'] == '1'}">selected</c:if>>
											已读
										</option>
									</select>
								</li>
								<li>
									...
									<select name="Q_type_S" class="selectSearch">
										...
										<option value="-1"
											<c:if test="${param['Q_type_S'] == '-1'}">selected</c:if>>
											待办
										</option>
										<option value="21"
											<c:if test="${param['Q_type_S'] == '21'}">selected</c:if>>
											转办
										</option>
										<option value="15"
											<c:if test="${param['Q_type_S'] == '15'}">selected</c:if>>
											沟通意见
										</option>
										<option value="26"
											<c:if test="${param['Q_type_S'] == '26'}">selected</c:if>>
											代理
										</option>
										<option value="38"
											<c:if test="${param['Q_type_S'] == '38'}">selected</c:if>>
											流转意见
										</option>
									</select>
								</li>
								<li>
									...
									<select name="Q_status_N" class="selectSearch">
										...
										<option value="1"
											<c:if test="${param['Q_status_N'] == '1'}">selected</c:if>>
											审批中
										</option>
										<option value="5"
											<c:if test="${param['Q_status_N'] == '5'}">selected</c:if>>
											已撤销
										</option>
										<option value="6"
											<c:if test="${param['Q_status_N'] == '6'}">selected</c:if>>
											已驳回
										</option>
									</select>
								</li>
								<div class="row_date">
									<li>
										...
										<input type="text" name="Q_beginCreateTime_DL"
											class="datePicker inputText" datetype="1"
											value="${param['Q_beginCreateTime_DL']}" />
									</li>
									<li>
										<span class="label">至: </span>
										<input type="text" name="Q_endCreateTime_DG"
											class="datePicker inputText"
											value="${param['Q_endCreateTime_DG']}" datetype="2" />
									</li>
								</div>
							</ul>
			...
			<div class="panel-body">
				...
				<display:table name="taskList" id="taskItem"
					requestURI="pendingMattersList.ht" sort="external" cellpadding="1"
					...
					<div style="background-color: red">
						<display:column title="${checkAll}" media="html"
							...
							<c:choose>
								<c:when
									test="${taskItem.description eq '15' || taskItem.description eq '38'}">
									<input type="checkbox" class="pk" name="id" disabled="disabled"
										value="${taskItem.id}">
								</c:when>
								<c:otherwise>
						...
						<display:column title="事项名称" sortable="true" sortName="subject"
							...
							<c:choose>
								<c:when test="${taskItem.description eq '36'}">
									<a name="subject" href="#${taskItem.id}"
										onclick="javascript:restartTask(${taskItem.id})"
										style="color:${warningSet[taskItem.priority].color}"
										title='${taskItem.subject}'
										<c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if>
										<c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if>>${f:subString(taskItem.subject)}</a>
								</c:when>
								<c:otherwise>
									<a name="subject" href="#${taskItem.id}"
										onclick="javascript:executeTask(${taskItem.id})"
										style="color:${warningSet[taskItem.priority].color}"
										title='${taskItem.subject}'
										<c:if test="${taskItem.hasRead == 0}"> class='message close-message'</c:if>
										<c:if test="${taskItem.hasRead != 0}"> class='message open-message'</c:if>>${f:subString(taskItem.subject)}</a>
						...
						<display:column title="创建时间" sortable="true"
							...
							<fmt:formatDate value="${taskItem.createTime}"
