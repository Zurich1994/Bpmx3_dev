...
<%
	...
	String postUrl = basePath + "/platform/bpm/bpmDefinition/flexDefSave_b.ht?isAjaxRequest=true";
	String postUrl1= basePath + "/platform/bpm/bpmDefinition/flexDefSave1.ht?isAjaxRequest=true";
	String postUrl15= basePath + "/platform/bpm/bpmDefinition/flexDefSave15.ht?isAjaxRequest=true";
	String postUrl0= basePath + "/platform/bpm/bpmDefinition/flexDefSave00.ht?isAjaxRequest=true";	
	String flowKeyGetUrl = basePath+"/platform/bpm/bpmDefinition/getFlowKey_b.ht";
	String flowListGetUrl = basePath+"/platform/bpm/bpmDefinition/getFlowListByTypeId_b.ht";
	String flagflex=request.getAttribute("flagflex").toString();
	
    String xmlRecord = request.getAttribute("xmlRecord").toString();
  
	String postUrl11= basePath + "/platform/bpm/bpmDefinition/flexbs.ht?isAjaxRequest=true";
	...
	if(defIdObj != null && !defIdObj.toString().equals("0")){
		...
		loadDateUrl = basePath + "/platform/bpm/bpmDefinition/flexGet_b.ht?temp=g&defId=" + defId;
	}
	
	String postUrl3 = basePath + "/platform/bpm/bpmDefinition/flexDynamicShow.ht?isAjaxRequest=true&defId=" + defId;
	String postUrl4 = basePath + "/platform/bpm/bpmDefinition/flexPictureShow.ht?isAjaxRequest=true";
...
%>
  ...
  <head>
  	<title>
  		在线拓扑图设计--<c:choose>
  			<c:when test="${not empty bpmnetworkmap}">
  				${bpmnetworkmap.subject} --(版本${bpmnetworkmap.versionNo})
  			</c:when>
  			<c:otherwise>
  				${subject}
  			</c:otherwise>
  ...
  <style type="text/css" media="screen"> 
	...
	//供flex方向调用,设备利用率图表显示---史欣慧
	function getCtxPath14(){
    
		return document.getElementById("postUrl4").value;
	}
	//供flex方向调用,网络模板,wyx
	...
	//业务部署-那晓旭
	function getCtxPath11(){
    
		return document.getElementById("postUrl11").value;
	}
 ...
 <textarea id="flagflex" style="display:none"><%=flagflex%></textarea>
 	...
 	<textarea id="flagflex" style="display:none"><%=flagflex%></textarea>
  	...
  	<input id="postUrl3" type="hidden" value="<%=postUrl3%>" /> 
  	<input id="postUrl4" type="hidden" value="<%=postUrl4%>" />
  	<input id="postUrl15" type="hidden" value="<%=postUrl15%>" />
  	<input id="flexLoadDataUrl" type="hidden" value="<%=loadDateUrl%>" />
  	<input id="postUrl11" type="hidden" value="<%=postUrl11%>" />
  </div>
  <div id="flashContent">
