...
<script type="text/javascript"src="${ctx}/js/hotent/platform/system/SysQuerySettingFilter.js"></script>
...
<div class="hidden">
		...
		<select id="flowVarsSelect" class="left margin-set" name="flowVars" onchange="flowVarChange.apply(this)">
			...
			<c:forEach items="${sysQueryFields}" var="field">
				...
				<option class="field-item" source="${source}"  value="${field.name}" ftype="${field.type}" >${field.fieldDesc}</option>
			</c:forEach>
		</select>
</div>
</body>
