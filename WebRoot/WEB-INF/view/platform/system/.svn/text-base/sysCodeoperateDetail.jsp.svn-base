	...
	function initTable(){
		...
		try{
			...
			for(var i=0;i<nodes.length;i++){
				var node=nodes[i];
				//actDefId: "fbcs_operate1:1:10000047560021"
                //actDefKey: "fbcs_operate1"
                //actDeployId: 10000047560018
                var actDefId=node.actDefId
				var formDefId=node.defId;
				formDefIds[i]=formDefId;
				alert(formDefId);
				$.post("${ctx}/platform/form/bpmFormDef/getTableInfo1.ht?formDefId="+formDefId,function(data){
...
<body>
	<div class="panel h100">
		...
		<form method="post" id="codeForm"  action="codeoperateZip.ht" class="h100" >
