		...
		<div id="defLayout" >
	            <div position="left" title="获取字段列表" style="overflow: auto;width: 300px;height:400px;">
					<table cellpadding="1" class="table-grid table-list" cellspacing="1">
						...
						<c:forEach var="field" items="${bpmFormFields }" varStatus="status">
						...
						<tr class="${clsName}">
							<td>
								<c:choose>
									...
									<c:otherwise>
										<input type="checkbox" name="fieldName" class="pk"  value="${field.fieldName }" id="${field.fieldName }" dbType="${field.fieldType }">
									</c:otherwise>
								</c:choose>
							</td>
							<td nowrap="nowrap">
							...
							<td>
								<input type="text" name="comment" class="inputText" value="${field.fieldDesc }">
							</td>
						</tr>
						</c:forEach>
					</table>
