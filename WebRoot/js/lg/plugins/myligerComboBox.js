           	  ...
           	  //没有复选框。
	              	...
	              	var onClick = function(event, treeId, treeNode) {
	              		if(!treeNode.isParent){
	              			$.each($("#all").find("*"), function(idx,obj) {
		                    	if($(obj).attr("name")=="disable"){
		                    		$(obj).removeAttr("disabled");
		                    		$(obj).attr("onclick","");
		                    	}
		                    });
	              			$("#eventsave").click(saveByJson);
		              		$("#btnSearch").click(searchdefinition);
		                    $("#mybuttondialog").click(searchdialog);
		                    //$("#mybuttonform").click(searchform);
		                    //$("#mybuttondatatemplate").click(searchdatatemplate);
	              		
	              		if(treeNode.domId!=-1){
	              			var domstr = $(editor.getContent()).find(treeNode.domtag+"[domId="+treeNode.domId+"]").attr("myjson");
	              			var domjson = JSON.parse(domstr); 
	              			var varjson;
	              			var selects=[];
	              			var urlnew = __ctx+"/platform/bpm/bpmDefinition/getvarliststr.ht?defId="+domjson.graphId;
	              	    	var inputvar=[];
	              	    	var outputvar=[];
	              	    	if(domjson.readdom){
	              	    		readdom=domjson.readdom;
	              	    	}
	              	    	if(domjson.writedom){
		              	    	writedom=domjson.writedom;	              	    		
	              	    	}
	              	    	$.ajax({
	            			    type:"post",
	            			    async:false,
	                         ...
	                         if(domjson.readdom.length){
	                        	 $.each(domjson.readdom, function(idx,obj) {
		                 				input.push(obj.indomid.toString());
		                 				input2.push(obj.invarid.toString());
		                 	     }); 
	                         }
	                         if(domjson.writedom.length){
	                        	 $.each(domjson.writedom, function(idx,obj) {
	 	              	    		output.push(obj.outdomid.toString());
	 	              	    		output2.push(obj.outvarid.toString());
	 	                 	      }); 
	                         }
	              	    	tags.push("input");
	              	        tags.push("textarea");
	              	        typenames.push(":text");
	              	        typenames.push("");
	              	        textflag = getEventDomByTagName(tags,"intextframe",typenames,input);
	              	        getEventDomByTagName(tags,"outintextframe",typenames,output);
	              	        tags.length=0;
	              	        typenames.length=0;
	              	        tags.push("iframe");
	              	        typenames.push("");
	              	        iframeflag = getEventDomByTagName(tags,"iniframeframe",typenames,input);
	              	        getEventDomByTagName(tags,"outiniframeframe",typenames,output);
	              	        tags.length=0;
	              	        typenames.length=0;
	                 	     ...
	                 	     if(input.length){
	                 	    	 $('#textframe input[type=checkbox]:checked').each(function(){
	 	                            $("#"+$(this).attr("id").replace("eventtext","select").replace("in","invar")).removeAttr("disabled");
	 	                            var reg = /[a-zA-Z]/g;
	 	                            var hiddenid  = $("#hiddenid"+$(this).attr("id").replace(reg,"")).val();
	 	                            var index = $(this).attr("id").replace(reg,"");
	 	                            $("#"+$(this).attr("id").replace("eventtext","select").replace("in","invar")+" option").each(function(){
	 	                            	   if($(this).val().toString() == input2[input.indexOf(hiddenid)].toString()){
	 	                            		   	$("#invarid"+index).attr("value",$(this).val().toString());
	 	                            	    	$(this).attr("selected","true");
	 	                            	    	//alert($("#invarid"+$(this).attr("id").replace(reg,"")).attr("class"));
	 	                            	   }
	 	                            });
	 	                         });
	 	                 	   $('#iframeframe input[type=checkbox]:checked').each(function(){
	 	                            $("#"+$(this).attr("id").replace("eventtext","select").replace("in","invar")).removeAttr("disabled");
	 	                            var reg = /[a-zA-Z]/g;
	 	                            var hiddenid  = $("#hiddenid"+$(this).attr("id").replace(reg,"")).val();
	 	                            $("#"+$(this).attr("id").replace("eventtext","select").replace("in","invar")+" option").each(function(){
	                 	   ...
	                 	   if(output.length){
	                 		  $('#outtextframe input[type=checkbox]:checked').each(function(){
		                            $("#"+$(this).attr("id").replace("eventtext","select").replace("out","outvar")).removeAttr("disabled");
		                            var reg = /[a-zA-Z]/g;
		                            var hiddenid  = $("#hiddenid"+$(this).attr("id").replace(reg,"")).val().toString();
		                            $("#"+$(this).attr("id").replace("eventtext","select").replace("out","outvar")+" option").each(function(){
		                            	   if($(this).val().toString() == output2[output.indexOf(hiddenid)].toString()){
		                            	    	$(this).attr("selected","true");
		                            	   }
		                            });
		                         });
		                 	 $('#outiframeframe input[type=checkbox]:checked').each(function(){
		                            $("#"+$(this).attr("id").replace("eventtext","select").replace("out","outvar")).removeAttr("disabled");
		                            var reg = /[a-zA-Z]/g;
		                            var hiddenid  = $("#hiddenid"+$(this).attr("id").replace(reg,"")).val().toString();
		                            $("#"+$(this).attr("id").replace("eventtext","select").replace("out","outvar")+" option").each(function(){
	                 	   	...
	                 	   	$("#inputDom").attr("value",domjson.innum);
	                 	   	$("#outputDom").attr("value",domjson.outnum);
	              	    	$("#btnname").attr("value",domjson.domname);
	              			$("#definition").attr("value",domjson.graphsubject);
	              			$("#definitionid").attr("value",domjson.graphId);
	              			$("#buttondialogid").attr("value",domjson.dialogId);
	              			$("#buttondialogname").attr("value",domjson.dialogname);
	              			//$("#buttonformkey").attr("value",domjson.formKey);
	              			//$("#buttonformsubject").attr("value",domjson.formsubject);
	              			//$("#buttondatatemplateid").attr("value",domjson.templateId);
	              			//$("#buttondatatemplatesubject").attr("value",domjson.templatesubject);
	              			$("#texttype").attr("value",domjson.intextput);
	              			$("#iframetype").attr("value",domjson.inframeput);
	              			$("#outtexttype").attr("value",domjson.outtextput);
	              			$("#outiframetype").attr("value",domjson.outframeput);
	              			$("#selectevent option").each(function (){  
	              			    if($(this).val()==domjson.domaction){
	              			    	$(this).attr("selected","selected");
	              			    }    
	              			 });  
	              			if(domjson.graphsubject){
	              				$(".type").removeAttr("disabled");
	              			}
	              			$("#show").attr("href",__ctx+"/platform/bpm/bpmDefinition/design.ht?defId="+domjson.graphId+"&flagflex=3");  
	              		}else{
	              			$("#inputDom").attr("value",0);
	              	        $("#outputDom").attr("value",0);
	              			$("#btnname").attr("value","");
	              			$("#definition").attr("value","");
	              			$("#definitionid").attr("value","");
	              			$("#buttondialogid").attr("value","");
	              			$("#buttondialogname").attr("value","");
	              			//$("#buttonformkey").attr("value","");
	              			//$("#buttonformsubject").attr("value","");
	              			//$("#buttondatatemplateid").attr("value","");
	              			//$("#buttondatatemplatesubject").attr("value","");
	              			$("#texttype").attr("value","");
	              			$("#iframetype").attr("value","");
	              			$("#outtexttype").attr("value","");
	              			$("#outiframetype").attr("value","");
	              			$("#definitionid").attr("value","");
	              			$("#selectevent option").each(function (){  
	               		...
	               		$("#pId").val(treeNode.getParentNode().domId);
	               		$("#domId").val(treeNode.domId);
	               		g._changeValue(value.join(p.split), text.join(p.split));
	               	};
	               	//外部传入点击事件
