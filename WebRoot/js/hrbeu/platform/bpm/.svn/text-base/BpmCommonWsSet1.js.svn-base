...
$(function() {
	// 布局
	$("#selectSys").click(function()
	{
		SubSystemDialog({isSingle:1,callback:function(systemId,systemName,alias){
		    $("#subsystem").val(systemName);
...
function saveWebserviceSet(){
	...
	if(!wsTree)
		{
		$.ligerDialog.warn('请先解析wsdl');
		return;
		}
	var nodes = wsTree.getSelectedNodes();
	if(nodes[0]==null)
		{
				$.ligerDialog.warn('请先选择方法');
				return;
		}
	...
	$.post(url,{setObj:setObj,customParams:customParams},function(d){
		...
		if(result.result)
			$.ligerDialog.success(result.message,"提示信息",function(){
				...
				var url=__ctx+'/platform/bpmCommonWsSet/bpmCommonWsSet/test.ht?setId='+result.wsid;
...
var addMethodNode = function(t,depParse) {
	...
	$.ajax({
		...
		success : function(result) {
			...
			$("<th>服务测试别名</th>").appendTo("#testname");
			$("<td colspan=\"3\"><input type=\"text\" name=\"alias\"  /></td>").appendTo("#testname");
			
			$("<th style=\"width: 15%;\">命名空间</th>").appendTo("#tr");
			 $("<td style=\"width: 35%;\" var=\"namespace\"></td>").appendTo("#tr");
			 $("<th style=\"width: 15%;\">调用方法</th>").appendTo("#tr");
			 $("<td style=\"width: 35%;\" var=\"method\"></td>").appendTo("#tr");
			 $("[var=namespace]", operatorField).html(json.namespace);
			 $("[var=method]", operatorField).html(json.method);
			setTimeout(function(){
...
function loadWsTree() {
	...
	$.ajax({
				type : "POST",
				url : __ctx + "/reg/mregwsdl/mregWsdl/getTreeData.ht",
				data : {
					'wsdlUrl' : $('#wsdlTxt').val()
				},
				dataType : "json",
