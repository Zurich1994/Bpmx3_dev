			...
			.each(
					function() {
						var me = $(this), methodInfo = me.data("methodInfo"), inputDiv = $(
								"div[var=inputTreeEdit]", me), ouputDIv = $(
								"div[var=outputTreeEdit]", me), inputTable = $(
								".table-detail[zone=binding]", inputDiv), outputTable = $(
								".table-detail[zone=binding]", ouputDIv)
						obj = {};
	...
	$
			.ajax({
				...
				success : function(result) {
					...
					// 对应的编辑处理div
					$("[var=outputTreeEdit]", operatorField).attr("id",
							$outputTreeUl.attr("id") + "-edit");
					...
					setTimeout(function() {
						...
						if (depParse) {
							var inputs = t.inputs, outputs = t.outputs;
							// 解析输入参数绑定
...
function cloneBindingTable(t, d, depParse) {
	...
	if ($editTable.length < 1) {
		...
		if ("outputTreeEdit" == divType) {
			...
			$("select[name=bindingType]", $editTable).find("option").each(
					function() {
						if ($(this).val() == "1")
							$(this).remove();
					});
