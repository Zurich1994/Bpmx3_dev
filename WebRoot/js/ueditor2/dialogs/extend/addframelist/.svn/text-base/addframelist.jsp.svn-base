...
//			}
			...
			var subclass = cla.substring(7);
			var html = '<span name="link-frame" style="display:inline-block;"  onclick = "sheriframe(\''+iname+'\',\''+subclass+'\');">';
			var label = $("#a-label").val();
			html += '<a href="javascript:;" ';
			if(cla)html+='class="'+cla+'"';
			if(json)html+='datatemplate="'+json+'"';
			html +='>' + label + '</a>';
			html += '</span> ';
			if(_element){
		...
		function getiframe(){
			 var ihtml = $(editor.getContent()).find("iframe");
			 var i,k,sel,opt;
		     for(i = 0;i<ihtml.length;i++){
		         k=i+1;
		    	 iname = $(ihtml[i]).attr("name");
		         opt = new Option();
		         opt.text=iname;
		         opt.value=k;
		         sel = document.getElementById("selectiframe");				    
				 sel.add(opt);
		    }
		};
		function getSelectValue(){
			var dia=$("#selectiframe option:selected").text();
			iname = dia;
			alert("选中的IFRAME名称是："+dia);
		};
--></script>
</head>
<body>
	...
	<div id="inputPanel">
		<fieldset class="base">
			...
			<table>
				...
				<tr>
					&nbsp;<select name="selectiframe" id="selectiframe"  onChange="getSelectValue();">
                      <option value ="0">-----------IFRAME选择------------</option>
                    </select>
				</tr>
			</table>
		</fieldset>
			...
			<table>
				<tr>
					...
					<input type="text" id="Bsubject" class="inputText" name="Bsubject" value="" readonly="readonly">
						...
						<input type="text" id="Bid"  class="inputText" name="Bid" value="" hidden="hidden">
					 
					<select id="dialog-type" onchange="dialogChange()" style="display: none">
				...
				<tr>
					<td colspan="3">
						...
						<table id="fieldTable" width="400px"></table>
						</div>
					</td>
				</tr>
			</table>
	...
	<#list data as a>
		...
		<tr class="fieldTr">
			...
			<td>
				...
				<!-- 主表字段 -->
				<input type="text" class="mainField" style="width:120px" value="\${a.name}" readonly id="\${a.id}"/>&nbsp;
