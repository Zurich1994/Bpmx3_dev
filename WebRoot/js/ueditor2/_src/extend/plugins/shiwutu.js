...
UE.plugins['input'] = function() {
	var me = this,
     ...
     optionHtml = ['<div id="panel" class="s">',
                   ...
                   '<label style="margin:0 0 0 10px;">选项: <input name="optionValue" class="long" type="text"/></label><a href="###" class="remove-bnt" tabindex="-1">移除</a></div>',
                   '</td>',
			...
			execCommand : function(cmdName,data,old) {
				if(data){
					if(old){
						...
						insertControl(cmdName,data,'<input type="text"  name="input:hidedomain:'+data[0].val+'"/>');
					}
				}
				else{
			...
			execCommand : function(cmdName,data,old) {
				if(data){
					if(old){
						...
						insertControl(cmdName,data,'<input type="text"  name=input:text:'+data[0].val+'" '+previewstyle+'/>');
					}
				}
				else{	
					dataType=[
			...
			execCommand : function(cmdName,data,old) {
				if(data){					
					if(old){
						...
						insertControl(cmdName,data,'<textarea name="m:textarea:'+data[0].val+'" cols="'+colNum+'" rows="'+rowNum+'"></textarea>');
					}
				}
				else{	
					dataType=[
					...
					me.curInputHtml=null;
					var html='<table class="edit_table"><tr class="style_tr"><th>列数:</th><td><input type="text" eid="cols"  style="width:175px; "/></td></tr><tr><th>行数:</th><td><input type="text" eid="rows" style="width:175px; "/></td></tr></table>';
					me.curInputHtml=getContentHtml(html);
					initDialog(cmdName);
				}				
			},
					...
					for(var i=0,c;c=data[i++];){
						if(c.id=='options'){
							...
							for(var j=0,m;m=options[j++];){
								text.push('<label><input name="m:option:'+m.value+'" type="checkbox" value="'+m.key+'"/>'+m.value+'</label>');
							}
						}
					}
					if(old){
...
//					html+='<tr><th>下拉框选项</th><td colspan="3"><span style="color:green;">每行对应一个选项<br /></span><textarea eid="options" cols="50" rows="5"></textarea></td></tr>';
	...
	function insertControl(cmdName,data,text){
		var html=['<span name="editable-input" myname="span:name:'+data[0].val+'" style="display:inline-block;padding:2px;" class="'+cmdName+'" external="'];					
	...
	function getContentHtml(html){
		//alert("单行文本");
		var content=['<table class="edit_table"><tr><th>控件注释:</th><td><input eid="comment" type="text" validate="{required:true}"  onblur="getKeyName(this)"/></td></tr><tr><th>控件名称:</th><td><input name"  validate="{required:true,varirule:true}"/></td></tr></table>'];
		//<tr><th>数据类型:</th><td>
