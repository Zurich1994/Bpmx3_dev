...
//            dom.ontouchstart = onclick;
    ...
    getMenuItems: function(graph, data, evt){
      ...
      if (data) {
        ...
        if("transactionChart"==data.typeName){//事务图右键
			if("true"==data.pageskip||"true"==data.readpage||"true"==data.writepage||"true"==data.datainquire||"true"==data.dataadd||"true"==data.datachange||"true"==data.datadelate){
				
				items.push({
								text:"设置节点属性",
								action: function() {
								
								var url=__ctx + "/platform/bpm/bpmDefinition/flex11.ht?&bpmDefinition.nodeLabel="+data.name+"&bpmDefinition.defId2=" +data.bpmNodeId+"&bpmDefinition.defId="+data.defId;
								$.post(url,{},function(resulturl){
			...
			if("true"==data.word){
				items.push({
				               ...
				               var url=__ctx + "/platform/bpm/bpmDefinition/flex107.ht?&bpmDefinition.defId2=" +data.bpmNodeId+"&bpmDefinition.defId="+data.defId;
				                $.post(url,{},function(resulturl){
					                window.open(resulturl);
				                     });
				                  }}), 
			...
			if(data.mail=="true"){
				items.push({
					text:"消息参数",
					action: function() {
					var url=__ctx + "/platform/bpm/bpmDefinition/flex109.ht?&bpmDefinition.defId2=" +data.bpmNodeId+"&bpmDefinition.defId="+data.defId;
					$.post(url,{},function(resulturl){
						window.open(resulturl);
					});
					}}),
			...
			if(data.webService=="true"){
				items.push({
					text:"webService原子操作",
					action: function() {
					var url=__ctx + "/platform/bpm/bpmDefinition/flex111.ht?&bpmDefinition.defId2=" +data.bpmNodeId+"&bpmDefinition.defId="+data.defId;
					$.post(url,{},function(resulturl){
						window.open(resulturl);
					});
					}}),
			...
			if("ParallelGateway"==data.bpmNodeType||"InclusiveGateway"==data.bpmNodeType||"ExclusiveGateway"==data.bpmNodeType){
				items.push({
					text:"设置分支条件",
					action: function() {
					var url=__ctx + "/platform/bpm/bpmDefinition/setCondition1.ht?deployId="+data.deployId+"&defId="+data.defId+"&nodeId="+data.bpmNodeId+"&parentActDefId="+data.parentActDefId;
			...
			if("CallActivity"==data.bpmNodeType){
				items.push({
					text:"查看外部子图",
					action: function() {
					var url=__ctx + "/platform/bpm/bpmDefinition/flex106.ht?&bpmDefinition.defKey=" +data.defKey+"&bpmDefinition.proTypeName="+data.typeName;
					$.post(url,{},function(resulturl){
						window.open(resulturl);
					});
					}}),
			...
			if(data.subDefKey!=""&&data.subDefKey!=null){
				items.push({
					text:"查看事务图",
					action: function() {
					var url=__ctx + "/platform/bpm/bpmDefinition/flex105.ht?&bpmDefinition.defKey=" +data.defKey+"&bpmDefinition.proTypeName="+data.typeName;
					$.post(url,{},function(resulturl){
						window.open(resulturl);
					});
					}}),
					items.push({
						text:"主从参数绑定",
						action: function() {
					 var url= __ctx +"/platform/bpm/bpmDefinition/flexan.ht?actDefId=" +data.actDefId+"&nodeId=" + data.bpmNodeId+"&defId=" +data.defId;
					 window.open(url);
						}}),
						items.push(Q.PopupMenu.Separator);
				}
			}
			if("operationChart"==data.typeName){        //操作图右键
				if("ParallelGateway"==data.bpmNodeType||"InclusiveGateway"==data.bpmNodeType||"ExclusiveGateway"==data.bpmNodeType){
					items.push({
						text:"设置分支条件",
						action: function() {
				...
				if(data.startSubFlow=="true"){
					items.push({
						text:"查看外部子图",
						action: function() {
						var url=__ctx + "/platform/bpm/bpmDefinition/flex106.ht?&bpmDefinition.defKey=" +data.subDefKey+"&bpmDefinition.proTypeName="+data.typeName;
						$.post(url,{},function(resulturl){
							window.open(resulturl);
						});
						}}),
				...
				if(data.bpmNodeType=="UserTask"){
					items.push({
						text:"加载报表",
						action: function() {
						var url=__ctx + "/platform/system/reportTemplate/list1.ht?&bpmDefinition.defKey=" +data.subDefKey+"&bpmDefinition.proTypeName="+data.typeName;
				...
				if(data.affair=="true"){
					items.push({
						text:"查看事务图",
						action: function() {
						var url=__ctx + "/platform/bpm/bpmDefinition/flex105.ht?&bpmDefinition.defKey=" +data.subDefKey+"&bpmDefinition.proTypeName="+data.typeName;
						$.post(url,{},function(resulturl){
							window.open(resulturl);
						});
						}}),
				...
				if("UserTask"==data.bpmNodeType){
					items.push({
						text:"查看页面",
						action: function() {
						var url=__ctx + "/platform/bpm/bpmDefinition/flex104.ht?&bpmDefinition.defId=" +data.defId+"&bpmDefinition.defId2="+data.bpmNodeId;
						$.post(url,{},function(resulturl){
							window.open(resulturl);
						});
						}}),
			...
			if("flowChart"==data.typeName){   //流程图右键
				if("ParallelGateway"==data.bpmNodeType||"InclusiveGateway"==data.bpmNodeType||"ExclusiveGateway"==data.bpmNodeType){
					items.push({
						text:"设置分支条件",
						action: function() {
				...
				var name1=data.name
				if(name1.indexOf("脚本")>-1||"true"==data.word){
					items.push({
									text:"设置脚本",
									action: function() {
				...
				if("UserTask"==data.bpmNodeType&&data.user=="true"){
					items.push({
						text:"查看操作图",
						action: function() {
						var url=__ctx + "/platform/bpm/bpmDefinition/flex103.ht?&bpmDefinition.defKey=" +data.subDefKey+"&bpmDefinition.proTypeName="+data.typeName;
						$.post(url,{},function(resulturl){
							window.open(resulturl);
						});
						}}),
						...
						items.push({
							...
							var url=__ctx + "/equipment/equipment/equipment/applicationTree.ht?equipmentId=10000065370018";
				...
				if(data.mail=="true"){
					items.push({
						...
						items.push(t.PopupMenu.Separator);
					}
			}
        items.push({
