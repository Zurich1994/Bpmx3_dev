...
(function($) {
    $.widget("ui.portlet", {
        ...
        _create: function() {
            ...
            $.each(o.columns, function(ci, c) {
                ...
                $.each(c.portlets, function(pi, p) {
                    ...
                    item.attr('title',p.title);
                    item.attr('moreurl',p.more);
                    ...
                    if(p.more){
                    	var url='"'+p.more+'"';
                    	title.prepend("<a href='#' onclick='getMore("+url+")' class='ui-corner-all ui-portlet-event'><span class='ui-icon ui-icon-folder-open'></span></a>");
                    }
                    // content
                    var ct = $('<div/>', {
        ...
        _sortable: function(value) {
            ...
            var st = $(".ui-portlet-column", element).sortable({
                connectWith: ".ui-portlet-column",
                start:function(e,u){
                	var colNum=element.parent().index();
                	var sn=element.index();
                	if($.isFunction(o.beforeDrag)){
                		o.beforeDrag(colNum,sn);
                	}
                },
                update:function(e,u){
                	if($.isFunction(o.afterDrag)){
                		o.afterDrag();
                	}
                }
            }).disableSelection();
        ...
        _destoryItem: function(event) {
            ...
            var i=item.index();
            var j=item.parents('.ui-sortable').index();
            item.remove();
