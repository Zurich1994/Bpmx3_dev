...
DataRightsApp.service('dataRightsService', ['$http','$jsonToFormData','BaseService', function($http,$jsonToFormData,BaseService) {
	var service = {
		init:function($scope){
			...
			$scope.managerButtons = [{k:"add",v:"新增"},{k:"edit",v:"编辑"},{k:"del",v:"删除"},{k:"detail",v:"明细"},{k:"export",v:"导出"},{k:"import",v:"导入"},{k:"start",v:"启动流程"},{k:"reStart",v:"重启流程"},{k:"bangPic",v:"绑定事务图"}];
		...
		showResponse : function(responseText){
			...
			if (obj.isSuccess()) {
				$.ligerDialog.confirm( obj.getMessage()+",是否继续操作","提示信息", function(rtn) {
					if(rtn){
						//var id =10000058840050;
						var id=DataRightsJson.tableId;
						var formKey=DataRightsJson.formKey;
						
						window.location.href =  "edit.ht?formKey="+formKey+"&tableId="+id;//location.href.getNewUrl();
					}else{
