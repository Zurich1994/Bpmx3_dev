...
/*显示租户信息*/
function showZh(){
	var zhid = $("[name='m:me_zlhtbg:zhID']").val();
	if(zhid == null || zhid == ""){
...
function checkDataBeforeInsert(data,tableName){
    if(tableName == 'me_zlhtbg_qtzkft' || tableName == 'me_zlhtbg_hykzkft'){
...
function changeTYShouYin(obj){
	var type = $("[name='m:me_zlhtbg:tysy']").val();
...
function zlsjChangeEvent(){
	...
	$("[name='s:me_zlhtbg_klzgz:sjd']").trigger("change");
	validateWyfDates();
	validateMysfxm();
}
...
/* 表租金标准信息 添加行事件*/
function me_zlhtbg_zjbzxxAddRowAfterEvent(row){
	var preRow = $(row).prev();
	/*获取原结算标准最后一个序号*/
	if(preRow.length==0 ||preRow.attr("style") == 'display: none;'){
		preRow = $(".listRow:visible",$("[tablename='me_zlhtbg_yzjbzxx']")).last();
	}
...
function validateZujinBiaozhunDates(curRow){
	//租赁有效期起，止
	var zuLinStartDate  =$("[name='m:me_zlhtbg:zlyxqq']").val();
	var zuLinEndDate  =$("[name='m:me_zlhtbg:zlyxqz']").val();
	...
	var rows = $(".listRow:visible",$("[tablename='me_zlhtbg_zjbzxx']"));
	for(var i=0,row;row=rows[i++];){
...
function calZongZuJin(obj){
	...
	var mianJi = $("[name='m:me_zlhtbg:jzmj']").val();
	if(!mianJi){
...
function createKoulvRules(){
	return;
	//租金标准信息
	var zjbzxx = $(".listRow:visible",$("[tablename='me_zlhtbg_zjbzxx']"));
	var kouLvs = $(".listRow:visible",$("[tablename='me_zlhtbg_klz']"));
	var klzggRows =$(".listRow:visible",$("[tablename='me_zlhtbg_klzgz']")); //扣率组规则
	var klzggTableDiv = $("div[tablename='me_zlhtbg_klzgg']");
	...
	for(var j=0,kouLv;kouLv=kouLvs[j++];){
		for(var i=0,row;row=zjbzxx[i++];){
			...
			var curRol = $(".listRow:visible",$("[tablename='me_zlhtbg_klzgz']"))[rowIndex++];
...
function handelKoulvTime(obj){
	...
	if(!shijianNo) return ;
	/*取的时间段*/
	var shiJianDuanNo =$("[name='s:me_zlhtbg_zjbzxx:sjd'][value="+shijianNo+"]");
...
function handelKoulvGroup(obj){
	...
	if(!koulvNo) return ;
	/*取扣率组*/
	var KouLv =$("[name='s:me_zlhtbg_klz:klzbh'][value="+koulvNo+"]");
	if(KouLv.length ==0) {
...
function initXSJEQvalidateRule(){ 
	var kouLvRules = $(".listRow:visible",$("[tablename='me_zlhtbg_klzgz']"));
	var combineMessage = [];
...
/*【物业费条款添加事件】me_zlhtbg_wyftk*/
function me_zlhtbg_wyftkAddRowAfterEvent(curRow){
	var preRow = $(curRow).prev();
	/*获取原结算标准最后一个序号*/
	if(preRow.length==0 ||preRow.attr("style") == 'display: none;'){
		preRow = $(".listRow:visible",$("[tablename='me_zlhtbg_ywyftk']")).last();
	}
...
function validateWyfDates(){
	...
	//循环计算开始日期
	var rows = $(".listRow:visible",$("[tablename='me_zlhtbg_wyftk']"));
	for(var i=0,row;row=rows[i++];){
		var shiJianDuan = $("[name$='sjd']",$(row)).val(i); //时间段
		if(i>1){
			//开始日期等于上个结束日期加1
...
function validateMysfxm (){
	...
	var sfxm = $(".listRow:visible",$("[tablename='me_zlhtbg_mysfxm']"));
	var sfxmArray =[];
...
/*结算标准信息   删除行事件*/
function me_zlhtbg_zjbzxxDelRowBeforeEvent(row){
...
function decomposeAllWyf(){
	decomposeAllJs('me_zlhtbg_wyftk','me_zlhtbg_wyffj');
}
function decomposeSingleWyf(){
	decomposeSingleJs('me_zlhtbg_wyftk','me_zlhtbg_wyffj');
}
function decomposeAllYzj(){
	decomposeAllJs('me_zlhtbg_zjbzxx','me_zlhtbg_ydzjfj');
}
function decomposeSingleYzj(){
	decomposeSingleJs('me_zlhtbg_zjbzxx','me_zlhtbg_ydzjfj');
}
...
function decomposeSingle(selectRow,fenJieTable){
	...
	//物业费
	if(fenJieTable == 'me_zlhtbg_wyffj'){
		var zj =FormUtil.commaback($("[name$='je']",selectRow).val());
		var sfgz =  $("[name='m:me_zlhtbg:wyfsfgzID']").val();
	//标准租金
	}else{
		var zj =FormUtil.commaback($("[name$='zj']",selectRow).val()); 
		var sfgz =$("[name='m:me_zlhtbg:zjsfgzID']").val();
	}
