...
FormDef.getParcelsByTableId = function(tableId){
...
//			
//		};
	var url=__ctx + "/platform/form/bpmFormDef/getAllParcelsByTableIdget.ht";
	var params={tableId:tableId};
	$.post('getAllParcelsByTableId.ht?tableId=' + tableId,function(data){	
		var json = eval('(' + data + ')');
//		var StruTree=$.fn.zTree.init($("#parceltree"), setting,json);
//		StruTree.expandAll(true);
		
		ptree=$.fn.zTree.init($("#parceltree"), setting,json);
	});
...
FormDef.getParcelsById = function(id){

	

	var url=__ctx + "/tableParcel/tableParcel/tableParcel/getbyJson";
	var params={id:id};
	$.ajax({
		type:"post",
		url:__ctx + "/tableParcel/tableParcel/tableParcel/getbyJson.ht",
		data:{id:id},
...
function updateptree(node){
	var name="wb";

	var newNode = node;

	var nodes=ptree.getNodes();
	var parentnode=nodes[0];

	newNode = ptree.addNodes(parentnode, newNode);
...
FormDef.showResponse = function(data) {
	...
	if (obj.isSuccess()) {// 成功
		$.ligerDialog.success('保存成功!', '提示', function() {
			...
			if(window.opener){ 
				//window.opener.dialog.get('pwin').reload();
