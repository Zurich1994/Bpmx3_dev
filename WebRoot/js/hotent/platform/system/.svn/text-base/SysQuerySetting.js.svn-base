...
SysQuerySettingEdit.prototype = {
	...
	getHtmlTd : function(v, rightId, rightName, rightScript) {
		var aryTd = [
				...
				'<a  href="javascript:;" name="btnScript" class="link var" title="常用脚本" onclick="__SysQuerySetting__.selectScript(this,false)">常用脚本</a>',
				'</span>'];
		return aryTd.join('');
	},
	/**
	...
	initManageField : function() {
		...
		var tr = $($("#manageTemplate .table-detail")[0]).clone(true,
				true);
	...
	filterDialog : function(conf) {
		...
		var url = __ctx
				+ "/platform/system/sysQuerySetting/filterDialog.ht?sqlId="
				+ conf.sqlId;
		url = url.getNewUrl();
