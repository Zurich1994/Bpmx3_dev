...
<script type="text/javascript">
	function startFlow(obj,id){
		...
		if(!linkObj.hasClass('disabled')) {
			...
			$.post("run.ht?isList=1&<#if table.isExternal==0>id<#else>${table.pkField}</#if>="+id+"&saveData=0&executeType=start",function(responseText){
		    ...
		    <display:table name="${classVar}List" id="${classVar}Item" requestURI="list.ht" sort="external" cellpadding="1" cellspacing="1" class="table-grid">
				...
				<display:column title="管理" media="html" style="width:220px">
					...
					<c:if test="<#noparse>${</#noparse>${classVar}Item.runId==0}">
						...
						<a href="javascript:;" onclick="startFlow(this,'<#noparse>${</#noparse>${classVar}Item.<#if table.isExternal==0>id<#else>${table.pkField}</#if>}')" class="link run"><span></span>提交</a>
					</c:if>
					<#else>
					<a href="del.ht?id=<#noparse>${</#noparse>${classVar}Item.<#if table.isExternal==0>id<#else>${table.pkField}</#if>}" class="link del"><span></span>删除</a>
					</#if>
